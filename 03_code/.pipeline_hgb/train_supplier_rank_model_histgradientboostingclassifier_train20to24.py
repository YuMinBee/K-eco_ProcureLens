#!/usr/bin/env python
"""Train a Hist Gradient Boosting supplier-ranking model from G2B item/supply data."""
...
SUPPLY_PATHS = [
    ...
    STANDARD / "supply_2020.csv",
    STANDARD / "supply_2021.csv"
]
AWARD_PATHS = [
    STANDARD / "조달청_나라장터_낙찰정보서비스_물품.csv",
    STANDARD / "award_2020.csv",
    STANDARD / "award_2021.csv"
]
...
def build_dataset(
    ...
    for event in events:
        ...
        if event.year in {"2020", "2021", "2022", "2023", "2024", "2025"} and event.item_code and event.winner_bizno:
            pool = item_pool(index, event.item_code, event.date)
            if event.winner_bizno in pool and len(pool) > 1:
                dataset = "train" if event.year in {"2020", "2021", "2022", "2023", "2024"} else "test"
...
class SimpleRegressionTree:
    ...
    def _build(self, x: np.ndarray, y: np.ndarray, indices: np.ndarray, depth: int) -> dict[str, Any]:
        targets = y[indices]
        prediction = float(np.mean(targets)) if len(targets) else 0.0

        if (
            depth >= self.max_depth
            or len(indices) < self.min_samples_leaf * 2
            or self.mse(targets) <= 1e-12
        ):
            return {"leaf": True, "pred": prediction}
        ...
        for feature in feature_indices:
            ...
            for _ in range(self.n_thresholds_per_feature):
                threshold = self.random_state.uniform(low, high)
                ...
                score = (len(left) * self.mse(y[left]) + len(right) * self.mse(y[right])) / len(indices)
                if score < best_score:
        ...
        gain = max(0.0, parent_error - best_score)
...
class SimpleHistGradientBoostingClassifier:
    def __init__(
        ...
        *,
        ...
        n_thresholds_per_feature: int = 8,
        random_seed: int = 42,
    ) -> None:
    ...
    def fit(self, x: np.ndarray, y: np.ndarray) -> None:
        ...
        eps = 1e-9
        self.base_score = math.log((positive + eps) / (negative + eps))
        ...
        for _ in range(self.n_estimators):
            ...
            tree = SimpleRegressionTree(
                ...
                n_thresholds_per_feature=self.n_thresholds_per_feature,
                random_state=random.Random(rng.randrange(1_000_000_000)),
            )
            tree.fit(x, residual)

            update = tree.predict_values(x)
...
def build_hist_gradient_boosting(args: argparse.Namespace) -> Any:
    if HistGradientBoostingClassifier is None:
        raise SystemExit("scikit-learn이 설치되어 있지 않습니다. HistGradientBoostingClassifier를 쓰려면 scikit-learn이 필요합니다.")
    print("모델 엔진: scikit-learn HistGradientBoostingClassifier")
...
def feature_importance_rows(model: Any, x: np.ndarray, y: np.ndarray) -> list[dict[str, Any]]:
    if permutation_importance is None:
        raise SystemExit("permutation_importance를 사용할 수 없습니다. scikit-learn 설치를 확인하세요.")
    ...
    importances = list(enumerate(result.importances_mean))
    importances.sort(key=lambda item: item[1], reverse=True)

    total = float(sum(abs(value) for _, value in importances)) or 1.0
    return [
        {
            "feature": FEATURE_COLUMNS[index],
            "importance": round(abs(float(value)) / total, 6),
...
def write_ai_rule_comparison(official_rows: list[dict[str, Any]]) -> None:
    ...
    write_csv(OUTPUT_DIR / "HGB_후보업체_AI_규칙점수_비교.csv", rows, columns)
    write_csv(
        OUTPUT_DIR / "HGB_후보업체_AI_TOP5_물품별.csv",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train a HistGradientBoosting supplier-ranking prototype.")
    parser.add_argument("--train-negatives", type=int, default=10)
    parser.add_argument("--test-negatives", type=int, default=30)
    parser.add_argument("--max-iter", type=int, default=200)  # HGB에서는 사실상 max_iter 역할
    ...
    model = build_hist_gradient_boosting(args)
    print("HistGradientBoosting 학습 중...")
    ...
    write_csv(OUTPUT_DIR / "histgradientboosting_evaluation_train20to24.csv", evaluation_rows, ["metric", "value"])
    write_csv(
        OUTPUT_DIR / "histgradientboosting_predictions_2025_top10.csv",
        prediction_rows,
        ["event_id", "rank", "AI예측점수", "label", "물품번호", "물품명", "사업자번호", "업체명", "개찰일자", "입찰공고번호"],
    )
    write_csv(
        OUTPUT_DIR / "histgradientboosting_feature_importance.csv",
        feature_importance_rows(model, test_x, test_y),
        ["feature", "importance", "raw_importance"],
    )
    official_rows = score_official_candidates(index, binary_sets, prior, model)
    write_csv(
        OUTPUT_DIR / "HGB_후보업체_AI예측점수.csv",
        official_rows,
        ["물품별_AI순위", "전체_AI순위", "candidate_id", "물품번호", "물품명", "사업자번호", "사업자번호_정규화", "업체명", "AI예측점수"] + FEATURE_COLUMNS,
    )
    write_ai_rule_comparison(official_rows)
    
    with (OUTPUT_DIR / "supplier_rank_histgradientboostingclassifier.pkl").open("wb") as f:
        pickle.dump({"model": model, "features": FEATURE_COLUMNS, "summary": summary, "metrics": metrics}, f)
    if joblib is not None:
        joblib.dump(
            {"model": model, "features": FEATURE_COLUMNS, "summary": summary, "metrics": metrics},
            OUTPUT_DIR / "supplier_rank_histgradientboostingclassifier.joblib",
        )
